...
contract PreauthorizedCalls is IPreauthorizedCalls, Calls {
  function preauthorizeCall(
    ...
    external
    meetsControllersThreshold(keccak256(abi.encode(_callRequestPreauthorized, _callRequestPreauthorization, _nonce, block.chainid)), _signatures)
  {
    require(_callRequestPreauthorization.maxCalls > 0, "CallRequestPreauthorization.maxCalls must not be 0");

    PreauthorizedCallsStorage.layout().callRequestPreauthorizations[
      keccak256(
        abi.encode(
  ...
  {
    PreauthorizedCallsStructs.CallRequestPreauthorization storage crp = getCallRequestPreauthorization(msg.sender, _callRequest);

    if (crp.maxCalls == 0) { // check if preauthorized for any caller.
    ...
    require(crp.lastCallTimestamp == 0 || crp.minCallInterval == 0 || (block.timestamp >= crp.lastCallTimestamp + crp.minCallInterval), "Call interval unmet");
  ...
  function getCallRequestPreauthorization(
    ...
    private
    view
    returns (PreauthorizedCallsStructs.CallRequestPreauthorization storage) 
  {
    return PreauthorizedCallsStorage.layout().callRequestPreauthorizations[
      keccak256(
        abi.encode(
          _caller,
          _callRequest.target,
